# v6 Qwen3-0.6B Commands (Fixed Values)

No fallback env vars are used here. These commands always run with:
...
RAY_ADDRESS=local \
RAY_TMPDIR=/tmp/ray_solver_v6_q306b \
PYTHONUNBUFFERED=1 \
python3 -m verl.trainer.main_ppo \
  ...
  trainer.rollout_data_dir='/workspace/verl/verl/qwen3_0.6b_dual_solver_v6_b16_n10' \
  trainer.logger='["console","wandb"]' \
  trainer.project_name='verl_grpo_v6_dual_loop_q306b' \
  trainer.experiment_name='qwen3_0.6b_dual_solver_v6_b16_n10' \
  ...
  | tee qwen3_0.6b_dual_solver_v6_b16_n10.log
```
...
RAY_ADDRESS=local \
RAY_TMPDIR=/tmp/ray_questioner_v6_q306b \
PYTHONUNBUFFERED=1 \
python3 -m verl.trainer.main_ppo \
  algorithm.adv_estimator=grpo \
  data.train_files=$HOME/data/gsm8k/train.parquet \
  ...
  trainer.critic_warmup=0 \
  trainer.log_val_generations=0 \
  trainer.rollout_data_dir='/workspace/verl/verl/qwen3_0.6b_dual_questioner_v6_b16_n10' \
  trainer.logger='["console","wandb"]' \
  trainer.project_name='verl_grpo_v6_dual_loop_q306b' \
  trainer.experiment_name='qwen3_0.6b_dual_questioner_v6_b16_n10' \
  trainer.n_gpus_per_node=1 \
  trainer.nnodes=1 \
  +data.apply_chat_template_kwargs.enable_thinking=False \
  trainer.save_freq=200 \
  ...
  trainer.total_training_steps=1000 \
  trainer.total_epochs=1000 \
  | tee qwen3_0.6b_dual_questioner_v6_b16_n10.log
```
