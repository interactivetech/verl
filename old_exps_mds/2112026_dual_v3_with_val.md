# 2112026 Dual Loop v3 + GSM8K Validation (Solver)

This variant enables periodic GSM8K validation on the solver while keeping cycle-gated N=2 flow stable.

Key changes vs `2112026_dual_v3.md`:
- Solver validation enabled (`trainer.test_freq=50`, `trainer.val_before_train=false`)
- Longer cycle/reward wait windows to absorb validation pauses
- Slightly slower polling to reduce churn while solver is in eval
...
export QUESTION_EXPERIMENT_NAME=qwen2_0.5b_dual_v5_cycle_n2_warm_val
export DUAL_LOOP_FAIL_ON_TIMEOUT=true
export DUAL_LOOP_CYCLE_TIMEOUT_S=900
...
export QUESTION_EXPERIMENT_NAME=qwen2_0.5b_dual_questioner_v5_n2_warm_val
...
# Avoid restoring stale dataloader iterator state
...
python3 -m verl.trainer.main_ppo \
  ...
  +custom_reward_function.reward_kwargs.wait_timeout_s=1800 \
  ...
  +custom_reward_function.reward_kwargs.cycle_timeout_s=900 \
...
## 3) Solver (start third, validation ON)

```bash
cd /workspace/verl/verl

export QUESTION_EXPERIMENT_NAME=qwen2_0.5b_dual_solver_v5_n2_warm_val
...
## 4) If validation causes timing stalls

Increase tolerance first (recommended):
- `DUAL_LOOP_CYCLE_TIMEOUT_S=1200`
- `+custom_reward_function.reward_kwargs.wait_timeout_s=2400`
- `+custom_reward_function.reward_kwargs.cycle_timeout_s=1200`

Reduce validation pressure second:
- Change solver `trainer.test_freq=100` or `200`
- Keep `trainer.val_before_train=false` (avoids startup stall)
- Keep `trainer.log_val_generations=0` or `1`
...
Healthy signal during validation-enabled runs:
- `solver_queue_size` may briefly rise during eval windows, then drain
- `candidate_counts.submitted` and `candidate_counts.solved` both continue increasing
- `cycle_counts.failed` should stay low and not trend upward fast
