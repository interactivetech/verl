# 2122026 Dual v3 Validation N=4 (Qwen3-0.6B, no warm start)

Goal:
- Run dual-loop validation with 4 questions per cycle.
- Use `Qwen/Qwen3-0.6B` for both questioner and solver.
- Start from base model weights only (no pretrained checkpoint resume).

N=4 invariants:
- Questioner `actor_rollout_ref.rollout.n = 4`
- Questioner reward `expected_cycle_size = 4`
- Questioner reward `min_required_per_cycle = 4`
- Solver `data.datagen.n_per_batch = 4`
- Solver `data.datagen.snapshot_size = 4`
- Solver `data.train_batch_size = 4`
- Solver `actor_rollout_ref.actor.ppo_mini_batch_size = 4`
...
export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_v3_cycle_n4_val
...
## 2) Questioner (N=4, no warm start)

```bash
cd /workspace/verl/verl

export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_questioner_v3_n4_val
...
CUDA_VISIBLE_DEVICES=1 RAY_ADDRESS=local RAY_TMPDIR=/tmp/ray_questioner_q3_n4 PYTHONUNBUFFERED=1 \
python3 -m verl.trainer.main_ppo \
  ...
  +custom_reward_function.reward_kwargs.expected_cycle_size=4 \
  +custom_reward_function.reward_kwargs.min_required_per_cycle=4 \
...
## 3) Solver (N=4, single-GPU style)

```bash
cd /workspace/verl/verl

export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_solver_v3_n4_val
...
CUDA_VISIBLE_DEVICES=0 RAY_ADDRESS=local RAY_TMPDIR=/tmp/ray_solver_q3_n4 PYTHONUNBUFFERED=1 \
python3 -m verl.trainer.main_ppo \
  algorithm.adv_estimator=grpo \
  data.train_files=$HOME/data/gsm8k/train.parquet \
  data.val_files=$HOME/data/gsm8k/test.parquet \
