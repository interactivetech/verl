# 2122026 Dual v3 Validation N=16 (Qwen3-0.6B, no warm start)

Goal:
- Run dual-loop validation with 16 questions per cycle.
- Use `Qwen/Qwen3-0.6B` for both questioner and solver.
- Start from base model weights only (no pretrained checkpoint resume).

N=16 invariants:
- Questioner `actor_rollout_ref.rollout.n = 16`
- Questioner reward `expected_cycle_size = 16`
- Questioner reward `min_required_per_cycle = 16`
- Solver `data.datagen.n_per_batch = 16`
- Solver `data.datagen.snapshot_size = 16`
- Solver `data.train_batch_size = 16`
- Solver `actor_rollout_ref.actor.ppo_mini_batch_size = 16`
...
export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_v3_cycle_n16_val
...
## 2) Questioner (N=16, no warm start)

```bash
cd /workspace/verl/verl

export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_questioner_v3_n16_val
...
CUDA_VISIBLE_DEVICES=1 RAY_ADDRESS=local RAY_TMPDIR=/tmp/ray_questioner_q3_n16 PYTHONUNBUFFERED=1 \
...
## 3) Solver (N=16, single-GPU style)

```bash
cd /workspace/verl/verl

export QUESTION_EXPERIMENT_NAME=qwen3_0.6b_dual_solver_v3_n16_val
...
CUDA_VISIBLE_DEVICES=0 RAY_ADDRESS=local RAY_TMPDIR=/tmp/ray_solver_q3_n16 PYTHONUNBUFFERED=1 \
python3 -m verl.trainer.main_ppo \
  algorithm.adv_estimator=grpo \
  data.train_files=$HOME/data/gsm8k/train.parquet \
  data.val_files=$HOME/data/gsm8k/test.parquet \
