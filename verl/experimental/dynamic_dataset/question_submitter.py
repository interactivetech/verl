...
"""
Submit GSM8K questions to a remote QuestionReceiver.

The receiver is expected to accept JSON at POST /submit with:
  {"questions": [{"question": "...", "answer": "...", "index": 0, "split": "train"}, ...]}

Environment variables:
  QUESTION_RECEIVER_URL: Target URL (default: http://127.0.0.1:8080/submit)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Submit GSM8K batches to QuestionReceiver.")
    parser.add_argument(
        "--receiver-url",
...
def questions(
    ...
    return {"questions": items, "split": split or _submitter.split}
...
def main() -> None:
    ...
    if args.serve:
        ...
        uvicorn.run("verl.experimental.dynamic_dataset.question_submitter:app", host=args.host, port=args.port)
        return
