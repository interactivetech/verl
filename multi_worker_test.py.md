...
```bash
#submitter
python3 -m verl.experimental.dynamic_dataset.question_submitter_v2 --serve --host 127.0.0.1 --port 8090
```

```bash
export QUESTION_EXPERIMENT_NAME=qwen2_0.5b_single_gpu_dual_remote4_v2
# can be:
# - all (default): logs submitter + submit + serve
# - incoming_only: logs only submitter + submit
# - none: disables questions_asked logging completely
# - serve_only: Keep only outgoing-to-trainer logs:
export QUESTION_ASKED_LOG_MODE=all
export QUESTION_SUBMITTER_URL='http://127.0.0.1:8090/questions'
python3 -m verl.experimental.dynamic_dataset.question_reciever_v2 --host 127.0.0.1 --port 8080
...
PYTORCH_ALLOC_CONF=expandable_segments:True,max_split_size_mb:64 PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    +data.datagen.snapshot_size=7473 \
    +data.datagen.block_until_full_snapshot=true \
    +data.datagen.refresh_each_epoch=true \
